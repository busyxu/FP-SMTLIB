(declare-fun b_ack!83 () (_ BitVec 32))
(declare-fun a_ack!84 () (_ BitVec 32))
(assert (not (bvslt b_ack!83 #x00000000)))
(assert (not (bvslt a_ack!84 #x00000000)))
(assert (not (= #x00000000 b_ack!83)))
(assert (not (= #x00000001 b_ack!83)))
(assert (not (= #x00000000 a_ack!84)))
(assert (not (= #x00000001 a_ack!84)))
(assert (not (bvslt (bvsub b_ack!83 a_ack!84) #x00000000)))
(assert (not (bvslt (bvsub b_ack!83 a_ack!84) #x00000000)))
(assert (= #x00000000 (bvsub b_ack!83 a_ack!84)))
(assert (not (bvslt (bvadd #x00000001 (bvsub b_ack!83 a_ack!84)) #x00000000)))
(assert (not (= #xffffffff (bvsub b_ack!83 a_ack!84))))
(assert (not (= #x00000000 (bvsub b_ack!83 a_ack!84))))

(check-sat)
(exit)
