(declare-fun c_ack!154 () (_ BitVec 64))
(declare-fun a_ack!155 () (_ BitVec 32))
(declare-fun b_ack!153 () (_ BitVec 32))
(declare-fun FPCHECK_FSUB_UNDERFLOW
             ((_ FloatingPoint 11 53) (_ BitVec 64))
             Bool)
(assert (not (fp.gt (fp.abs ((_ to_fp 11 53) c_ack!154))
            ((_ to_fp 11 53) #x40862642fefa39ef))))
(assert (not (bvslt b_ack!153 a_ack!155)))
(assert (not (bvslt a_ack!155 #x00000000)))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!154) ((_ to_fp 11 53) #x0000000000000000))))
(assert (= #x00000000 b_ack!153))
(assert (not (fp.lt (fp.abs ((_ to_fp 11 53) c_ack!154))
            ((_ to_fp 11 53) #x3e60000000000000))))
(assert (not (fp.leq (fp.abs ((_ to_fp 11 53) c_ack!154))
             ((_ to_fp 11 53) #x4008000000000000))))
(assert (fp.leq (fp.abs ((_ to_fp 11 53) c_ack!154))
        ((_ to_fp 11 53) #x4020000000000000)))
(assert (let ((a!1 (fp.sub roundNearestTiesToEven
                   (fp.div roundNearestTiesToEven
                           ((_ to_fp 11 53) #x4048000000000000)
                           (fp.abs ((_ to_fp 11 53) c_ack!154)))
                   ((_ to_fp 11 53) #x4026000000000000))))
  (FPCHECK_FSUB_UNDERFLOW
    (fp.mul roundNearestTiesToEven
            ((_ to_fp 11 53) #x4000000000000000)
            (fp.div roundNearestTiesToEven
                    a!1
                    ((_ to_fp 11 53) #x4014000000000000)))
    #xbff0000000000000)))

(check-sat)
(exit)
