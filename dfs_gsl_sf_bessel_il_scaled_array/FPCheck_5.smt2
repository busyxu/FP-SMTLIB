(declare-fun b_ack!25 () (_ BitVec 64))
(declare-fun a_ack!26 () (_ BitVec 32))
(declare-fun FPCHECK_FSUB_ACCURACY ((_ BitVec 64) (_ FloatingPoint 11 53)) Bool)
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!25) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.lt ((_ to_fp 11 53) b_ack!25) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (bvslt (bvadd #x00000001 a_ack!26) #x00000000)))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!25) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (= #xffffffff a_ack!26)))
(assert (= #x00000000 a_ack!26))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!25) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.lt (fp.abs ((_ to_fp 11 53) b_ack!25))
            ((_ to_fp 11 53) #x0028000000000000))))
(assert (fp.lt (fp.abs ((_ to_fp 11 53) b_ack!25)) ((_ to_fp 11 53) #x3fd0000000000000)))
(assert (FPCHECK_FSUB_ACCURACY #x8000000000000000 (fp.abs ((_ to_fp 11 53) b_ack!25))))

(check-sat)
(exit)
