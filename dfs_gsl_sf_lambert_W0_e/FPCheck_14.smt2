...
(assert (fp.gt ((_ to_fp 11 53) a_ack!20) ((_ to_fp 11 53) #x4008000000000000)))
(assert (let ((a!1 (fp.add roundNearestTiesToEven
                   (fp.sub roundNearestTiesToEven
                           (CF_log a_ack!20)
                           (CF_log (CF_log a_ack!20)))
                   (CF_log (CF_log a_ack!20)))))
  (fp.eq a!1 (CF_log a_ack!20))))
(assert (let ((a!1 (fp.sub roundNearestTiesToEven
                   (CF_log a_ack!20)
                   (fp.sub roundNearestTiesToEven
                           (CF_log a_ack!20)
                           (CF_log (CF_log a_ack!20))))))
  (fp.eq a!1 (CF_log (CF_log a_ack!20)))))
(assert (let ((a!1 (CF_exp (fp.sub roundNearestTiesToEven
                           (CF_log a_ack!20)
                           (CF_log (CF_log a_ack!20))))))
  (FPCHECK_FMUL_OVERFLOW
    (fp.sub roundNearestTiesToEven (CF_log a_ack!20) (CF_log (CF_log a_ack!20)))
    a!1)))
