(declare-fun c_ack!463 () (_ BitVec 64))
(declare-fun a_ack!464 () (_ BitVec 32))
(declare-fun FPCHECK_FMUL_UNDERFLOW ((_ BitVec 64) (_ BitVec 64)) Bool)
(declare-fun b_ack!462 () (_ BitVec 64))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (bvslt a_ack!464 #x00000000)))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x40862e42fefa39ef))))
(assert (not (= #x00000000 a_ack!464)))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x40862e42fefa39ef))))
(assert (not (= #x00000000 a_ack!464)))
(assert (not (= #x00000001 a_ack!464)))
(assert (fp.eq ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x0000000000000000)))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!463) ((_ to_fp 11 53) #x40862e42fefa39ef))))
(assert (not (= #x00000001 a_ack!464)))
(assert (= #x00000002 a_ack!464))
(assert (FPCHECK_FMUL_UNDERFLOW b_ack!462 b_ack!462))

(check-sat)
(exit)
