(declare-fun a_ack!175 () (_ BitVec 32))
(declare-fun b_ack!174 () (_ BitVec 64))
(declare-fun FPCHECK_FMUL_UNDERFLOW
             ((_ BitVec 64) (_ FloatingPoint 11 53))
             Bool)
(assert (not (bvslt a_ack!175 #x00000000)))
(assert (not (fp.leq ((_ to_fp 11 53) b_ack!174) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (= #x00000000 a_ack!175)))
(assert (not (= #x00000001 a_ack!175)))
(assert (not (= #x00000002 a_ack!175)))
(assert (fp.lt ((_ to_fp 11 53) b_ack!174) ((_ to_fp 11 53) #x4008000000000000)))
(assert (not (bvslt (bvadd #x00000001 a_ack!175) #x00000000)))
(assert (not (= #x00000000 (bvand (bvadd #x00000001 a_ack!175) #x00000001))))
(assert (let ((a!1 (ite (= ((_ extract 31 31) (bvadd #x00000001 a_ack!175)) #b1)
                (concat #b1 ((_ extract 31 1) (bvadd #x00000001 a_ack!175)))
                (concat #b0 ((_ extract 31 1) (bvadd #x00000001 a_ack!175))))))
  (not (= #x00000000 a!1))))
(assert (let ((a!1 (ite (= ((_ extract 31 31) (bvadd #x00000001 a_ack!175)) #b1)
                (concat #b1 ((_ extract 31 1) (bvadd #x00000001 a_ack!175)))
                (concat #b0 ((_ extract 31 1) (bvadd #x00000001 a_ack!175))))))
  (not (= #x00000000 (bvand a!1 #x00000001)))))
(assert (let ((a!1 (ite (= ((_ extract 31 31) (bvadd #x00000001 a_ack!175)) #b1)
                (concat #b1 ((_ extract 31 1) (bvadd #x00000001 a_ack!175)))
                (concat #b0 ((_ extract 31 1) (bvadd #x00000001 a_ack!175))))))
(let ((a!2 (= #x00000000
              (ite (= ((_ extract 31 31) a!1) #b1)
                   (concat #b1 ((_ extract 31 1) a!1))
                   (concat #b0 ((_ extract 31 1) a!1))))))
  (not a!2))))
(assert (let ((a!1 (ite (= ((_ extract 31 31) (bvadd #x00000001 a_ack!175)) #b1)
                (concat #b1 ((_ extract 31 1) (bvadd #x00000001 a_ack!175)))
                (concat #b0 ((_ extract 31 1) (bvadd #x00000001 a_ack!175))))))
(let ((a!2 (bvand (ite (= ((_ extract 31 31) a!1) #b1)
                       (concat #b1 ((_ extract 31 1) a!1))
                       (concat #b0 ((_ extract 31 1) a!1)))
                  #x00000001)))
  (not (= #x00000000 a!2)))))
(assert (let ((a!1 (ite (= ((_ extract 31 31) (bvadd #x00000001 a_ack!175)) #b1)
                (concat #b1 ((_ extract 31 1) (bvadd #x00000001 a_ack!175)))
                (concat #b0 ((_ extract 31 1) (bvadd #x00000001 a_ack!175))))))
(let ((a!2 (ite (= ((_ extract 31 31) a!1) #b1)
                (concat #b1 ((_ extract 31 1) a!1))
                (concat #b0 ((_ extract 31 1) a!1)))))
  (= #x00000000
     (ite (= ((_ extract 31 31) a!2) #b1)
          (concat #b1 ((_ extract 31 1) a!2))
          (concat #b0 ((_ extract 31 1) a!2)))))))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           (fp.mul roundNearestTiesToEven
                                   ((_ to_fp 11 53) #x3ff0000000000000)
                                   ((_ to_fp 11 53) b_ack!174))
                           (fp.mul roundNearestTiesToEven
                                   ((_ to_fp 11 53) b_ack!174)
                                   ((_ to_fp 11 53) b_ack!174)))
                   (fp.mul roundNearestTiesToEven
                           (fp.mul roundNearestTiesToEven
                                   ((_ to_fp 11 53) b_ack!174)
                                   ((_ to_fp 11 53) b_ack!174))
                           (fp.mul roundNearestTiesToEven
                                   ((_ to_fp 11 53) b_ack!174)
                                   ((_ to_fp 11 53) b_ack!174))))))
  (FPCHECK_FMUL_UNDERFLOW #x3ce0000000000000 (fp.abs a!1))))

(check-sat)
(exit)
