(declare-fun a_ack!1841 () (_ BitVec 64))
(declare-fun b_ack!1840 () (_ BitVec 64))
(declare-fun CF_log ((_ BitVec 64)) (_ FloatingPoint 11 53))
(declare-fun FPCHECK_FADD_ACCURACY ((_ BitVec 64) (_ FloatingPoint 11 53)) Bool)
(assert (not (fp.lt ((_ to_fp 11 53) a_ack!1841) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.lt ((_ to_fp 11 53) b_ack!1840) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!1840) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) a_ack!1841) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.leq ((_ to_fp 11 53) b_ack!1840)
             (fp.mul roundNearestTiesToEven
                     ((_ to_fp 11 53) #x3fe0000000000000)
                     ((_ to_fp 11 53) a_ack!1841)))))
(assert (not (fp.geq ((_ to_fp 11 53) a_ack!1841) ((_ to_fp 11 53) #x412e848000000000))))
(assert (fp.lt ((_ to_fp 11 53) a_ack!1841) ((_ to_fp 11 53) #x3fc999999999999a)))
(assert (fp.lt ((_ to_fp 11 53) b_ack!1840) ((_ to_fp 11 53) #x4014000000000000)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x3fe2788cfc6fb619)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #x3fe2788cfc6fb619))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x3fe2788cfc6fb619)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #x3fe2788cfc6fb619)))
(assert (let ((a!1 (fp.div roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #x3fe2788cfc6fb619)
                                   (CF_log b_ack!1840))
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #x3fe2788cfc6fb619)
                                   (CF_log b_ack!1840)))
                   ((_ to_fp 11 53) #x4018000000000000))))
  (fp.eq (fp.add roundNearestTiesToEven
                 (fp.sub roundNearestTiesToEven
                         ((_ to_fp 11 53) #x3fea51a6625307d3)
                         a!1)
                 a!1)
         ((_ to_fp 11 53) #x3fea51a6625307d3))))
(assert (let ((a!1 (fp.div roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #x3fe2788cfc6fb619)
                                   (CF_log b_ack!1840))
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #x3fe2788cfc6fb619)
                                   (CF_log b_ack!1840)))
                   ((_ to_fp 11 53) #x4018000000000000))))
  (fp.eq (fp.sub roundNearestTiesToEven
                 ((_ to_fp 11 53) #x3fea51a6625307d3)
                 (fp.sub roundNearestTiesToEven
                         ((_ to_fp 11 53) #x3fea51a6625307d3)
                         a!1))
         a!1)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #xbffc21c3e15d140a)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #xbffc21c3e15d140a))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #xbffc21c3e15d140a)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #xbffc21c3e15d140a)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x40106e22afa35265)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #x40106e22afa35265))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x40106e22afa35265)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #x40106e22afa35265)))
(assert (let ((a!1 (fp.abs (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbf2a01a01a01a01a)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc004101bf07ae56f)
                                   (CF_log b_ack!1840))))))
  (not (fp.eq a!1 ((_ to_fp 11 53) #x0000000000000000)))))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   ((_ to_fp 11 53) #xbf2a01a01a01a01a)
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc004101bf07ae56f)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.div roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbfff2a8ecc402949)
                                   (CF_log b_ack!1840)))
                   a!1)))
  (fp.eq a!2
         (fp.add roundNearestTiesToEven
                 ((_ to_fp 11 53) #xbfff2a8ecc402949)
                 (CF_log b_ack!1840))))))
(assert (let ((a!1 (fp.eq (fp.abs (fp.add roundNearestTiesToEven
                                  ((_ to_fp 11 53) #xbfff2a8ecc402949)
                                  (CF_log b_ack!1840)))
                  ((_ to_fp 11 53) #x0000000000000000))))
  (not a!1)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   ((_ to_fp 11 53) #xbf2a01a01a01a01a)
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc004101bf07ae56f)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.div roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbfff2a8ecc402949)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbfff2a8ecc402949)
                           (CF_log b_ack!1840)))))
  (fp.eq a!2 a!1))))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x3fefda591b88c85d)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #x3fefda591b88c85d))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x3fefda591b88c85d)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #x3fefda591b88c85d)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #xc00698442a97b713)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #xc00698442a97b713))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #xc00698442a97b713)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #xc00698442a97b713)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbec71de3a556c736)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc00698442a97b713)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00309ee7fc808cd)
                           (CF_log b_ack!1840)))))
  (not (fp.eq (fp.abs a!1) ((_ to_fp 11 53) #x0000000000000000)))))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbec71de3a556c736)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc00698442a97b713)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00309ee7fc808cd)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.div roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                   (CF_log b_ack!1840)))
                   a!1)))
  (fp.eq a!2
         (fp.add roundNearestTiesToEven
                 ((_ to_fp 11 53) #xbffea13fc987f9e0)
                 (CF_log b_ack!1840))))))
(assert (let ((a!1 (fp.eq (fp.abs (fp.add roundNearestTiesToEven
                                  ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                  (CF_log b_ack!1840)))
                  ((_ to_fp 11 53) #x0000000000000000))))
  (not a!1)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbec71de3a556c736)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc00698442a97b713)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00309ee7fc808cd)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.div roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbffea13fc987f9e0)
                           (CF_log b_ack!1840)))))
  (fp.eq a!2 a!1))))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #xbff61a6e5deab171)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #xbff61a6e5deab171))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #xbff61a6e5deab171)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #xbff61a6e5deab171)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbec71de3a556c736)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc00698442a97b713)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00309ee7fc808cd)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.abs (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                   (CF_log b_ack!1840))))))
  (not (fp.eq a!2 ((_ to_fp 11 53) #x0000000000000000))))))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbec71de3a556c736)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc00698442a97b713)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00309ee7fc808cd)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbff61a6e5deab171)
                           (CF_log b_ack!1840)))))
(let ((a!3 (fp.div roundNearestTiesToEven
                   a!2
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                   (CF_log b_ack!1840))))))
  (fp.eq a!3
         (fp.add roundNearestTiesToEven
                 ((_ to_fp 11 53) #xbff61a6e5deab171)
                 (CF_log b_ack!1840)))))))
(assert (let ((a!1 (fp.eq (fp.abs (fp.add roundNearestTiesToEven
                                  ((_ to_fp 11 53) #xbff61a6e5deab171)
                                  (CF_log b_ack!1840)))
                  ((_ to_fp 11 53) #x0000000000000000))))
  (not a!1)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbec71de3a556c736)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc00698442a97b713)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00309ee7fc808cd)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xbffea13fc987f9e0)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbff61a6e5deab171)
                           (CF_log b_ack!1840)))))
  (fp.eq (fp.div roundNearestTiesToEven
                 a!2
                 (fp.add roundNearestTiesToEven
                         ((_ to_fp 11 53) #xbff61a6e5deab171)
                         (CF_log b_ack!1840)))
         (fp.mul roundNearestTiesToEven
                 a!1
                 (fp.add roundNearestTiesToEven
                         ((_ to_fp 11 53) #xbffea13fc987f9e0)
                         (CF_log b_ack!1840)))))))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x4022a491a5eeb992)
                       (CF_log b_ack!1840))
               ((_ to_fp 11 53) #x4022a491a5eeb992))
       (CF_log b_ack!1840)))
(assert (fp.eq (fp.sub roundNearestTiesToEven
               (fp.add roundNearestTiesToEven
                       ((_ to_fp 11 53) #x4022a491a5eeb992)
                       (CF_log b_ack!1840))
               (CF_log b_ack!1840))
       ((_ to_fp 11 53) #x4022a491a5eeb992)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbe927e4fb7789f5e)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc007a1dc08ed251e)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00464a1c74169ae)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.abs (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc0011420a97c81ac)
                                   (CF_log b_ack!1840))))))
  (not (fp.eq a!2 ((_ to_fp 11 53) #x0000000000000000))))))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbe927e4fb7789f5e)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc007a1dc08ed251e)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00464a1c74169ae)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc0011420a97c81ac)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbffac979781a4eb6)
                           (CF_log b_ack!1840)))))
(let ((a!3 (fp.div roundNearestTiesToEven
                   a!2
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc0011420a97c81ac)
                                   (CF_log b_ack!1840))))))
  (fp.eq a!3
         (fp.add roundNearestTiesToEven
                 ((_ to_fp 11 53) #xbffac979781a4eb6)
                 (CF_log b_ack!1840)))))))
(assert (let ((a!1 (fp.eq (fp.abs (fp.add roundNearestTiesToEven
                                  ((_ to_fp 11 53) #xbffac979781a4eb6)
                                  (CF_log b_ack!1840)))
                  ((_ to_fp 11 53) #x0000000000000000))))
  (not a!1)))
(assert (let ((a!1 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbe927e4fb7789f5e)
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc007a1dc08ed251e)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xc00464a1c74169ae)
                           (CF_log b_ack!1840)))))
(let ((a!2 (fp.mul roundNearestTiesToEven
                   (fp.mul roundNearestTiesToEven
                           a!1
                           (fp.add roundNearestTiesToEven
                                   ((_ to_fp 11 53) #xc0011420a97c81ac)
                                   (CF_log b_ack!1840)))
                   (fp.add roundNearestTiesToEven
                           ((_ to_fp 11 53) #xbffac979781a4eb6)
                           (CF_log b_ack!1840)))))
  (fp.eq (fp.div roundNearestTiesToEven
                 a!2
                 (fp.add roundNearestTiesToEven
                         ((_ to_fp 11 53) #xbffac979781a4eb6)
                         (CF_log b_ack!1840)))
         (fp.mul roundNearestTiesToEven
                 a!1
                 (fp.add roundNearestTiesToEven
                         ((_ to_fp 11 53) #xc0011420a97c81ac)
                         (CF_log b_ack!1840)))))))
(assert (FPCHECK_FADD_ACCURACY #xbff21f1be4fba88a (CF_log b_ack!1840)))

(check-sat)
(exit)
