(declare-fun d_ack!467 () (_ BitVec 64))
(declare-fun b_ack!466 () (_ BitVec 32))
(declare-fun a_ack!469 () (_ BitVec 64))
(declare-fun c_ack!468 () (_ BitVec 64))
(assert (fp.eq ((_ to_fp 11 53) d_ack!467) ((_ to_fp 11 53) #x0000000000000000)))
(assert (bvsle #x00000000 b_ack!466))
(assert (not (bvsle #x00000001 b_ack!466)))
(assert (fp.eq ((_ to_fp 11 53) a_ack!469) ((_ to_fp 11 53) #x0000000000000000)))
(assert (let ((a!1 (fp.lt (fp.abs (fp.div roundNearestTiesToEven
                                  ((_ to_fp 11 53) c_ack!468)
                                  ((_ to_fp 11 53) #x3ff0000000000000)))
                  ((_ to_fp 11 53) #x3cb0000000000000))))
  (not a!1)))

(check-sat)
(exit)
