(declare-fun b_ack!573 () (_ BitVec 32))
(declare-fun a_ack!575 () (_ BitVec 32))
(declare-fun FPCHECK_FMUL_UNDERFLOW ((_ BitVec 64) (_ BitVec 64)) Bool)
(declare-fun c_ack!574 () (_ BitVec 64))
(assert (not (bvslt b_ack!573 #x00000000)))
(assert (not (bvslt a_ack!575 #x00000000)))
(assert (not (= #x00000000 b_ack!573)))
(assert (not (= #x00000001 b_ack!573)))
(assert (not (= #x00000000 a_ack!575)))
(assert (not (= #x00000001 a_ack!575)))
(assert (not (bvslt (bvsub b_ack!573 a_ack!575) #x00000000)))
(assert (not (bvslt (bvsub b_ack!573 a_ack!575) #x00000000)))
(assert (not (= #x00000000 (bvsub b_ack!573 a_ack!575))))
(assert (= #x00000001 (bvsub b_ack!573 a_ack!575)))
(assert (FPCHECK_FMUL_UNDERFLOW #x4000000000000000 c_ack!574))

(check-sat)
(exit)
