...
(assert (not (= #x00000001 b_ack!1948)))
(assert (= #x00000000 a_ack!1950))
(assert (not (bvslt b_ack!1948 #x00000000)))
(assert (not (= #x00000000 b_ack!1948)))
(assert (not (= #x00000001 b_ack!1948)))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!1949) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.gt (fp.abs ((_ to_fp 11 53) c_ack!1949))
            ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (bvslt #x00000001 b_ack!1948))
(assert (not (fp.gt (fp.abs ((_ to_fp 11 53) c_ack!1949))
            ((_ to_fp 11 53) #x7fefffffffffffff))))
(assert (bvslt #x00000002 b_ack!1948))
