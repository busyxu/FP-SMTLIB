...
(assert (not (bvsle a_ack!64 #x00000000)))
(assert (not (bvslt b_ack!63 #x00000000)))
(assert (not (bvslt (bvsdiv a_ack!64 #x00000002) b_ack!63)))
(assert (not (= #x00000000 b_ack!63)))
(assert (not (= #x00000002 a_ack!64)))
(assert (not (bvslt a_ack!64 #x00000015)))
(assert (= #x00000001 b_ack!63))
(assert (bvslt #x00000032 a_ack!64))
(assert (not (= #x00000000 (bvand a_ack!64 #x00000001))))
(assert (FPCHECK_FADD_ACCURACY
  ((_ to_fp 11 53) roundNearestTiesToEven (bvmul #x00000002 a_ack!64))
  #x3ff0000000000000))
