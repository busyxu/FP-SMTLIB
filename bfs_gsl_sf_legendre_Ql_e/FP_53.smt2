(declare-fun b_ack!890 () (_ BitVec 64))
(declare-fun a_ack!891 () (_ BitVec 32))
(assert (not (fp.leq ((_ to_fp 11 53) b_ack!890) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!890) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (bvslt a_ack!891 #x00000000)))
(assert (not (= #x00000000 a_ack!891)))
(assert (= #x00000001 a_ack!891))
(assert (not (fp.leq ((_ to_fp 11 53) b_ack!890) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!890) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (fp.lt (fp.mul roundNearestTiesToEven
                    ((_ to_fp 11 53) b_ack!890)
                    ((_ to_fp 11 53) b_ack!890))
            ((_ to_fp 11 53) #x3f6428a2f98d728d))))
(assert (not (fp.lt ((_ to_fp 11 53) b_ack!890) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (fp.lt ((_ to_fp 11 53) b_ack!890) ((_ to_fp 11 53) #x4018000000000000))))
(assert (fp.lt (fp.mul roundNearestTiesToEven
               ((_ to_fp 11 53) b_ack!890)
               ((_ to_fp 11 53) #x2000000000000000))
       ((_ to_fp 11 53) #x3fe24a5b5be85b8f)))

(check-sat)
(exit)
