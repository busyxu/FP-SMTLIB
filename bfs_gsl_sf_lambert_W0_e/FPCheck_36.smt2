...
(assert (fp.gt ((_ to_fp 11 53) a_ack!56) ((_ to_fp 11 53) #x4008000000000000)))
(assert (let ((a!1 (fp.add roundNearestTiesToEven
                   (fp.sub roundNearestTiesToEven
                           (CF_log a_ack!56)
                           (CF_log (CF_log a_ack!56)))
                   (CF_log (CF_log a_ack!56)))))
  (fp.eq a!1 (CF_log a_ack!56))))
(assert (let ((a!1 (fp.sub roundNearestTiesToEven
                   (CF_log a_ack!56)
                   (fp.sub roundNearestTiesToEven
                           (CF_log a_ack!56)
                           (CF_log (CF_log a_ack!56))))))
  (fp.eq a!1 (CF_log (CF_log a_ack!56)))))
(assert (let ((a!1 (CF_exp (fp.sub roundNearestTiesToEven
                           (CF_log a_ack!56)
                           (CF_log (CF_log a_ack!56))))))
  (FPCHECK_FMUL_UNDERFLOW
    (fp.sub roundNearestTiesToEven (CF_log a_ack!56) (CF_log (CF_log a_ack!56)))
    a!1)))
