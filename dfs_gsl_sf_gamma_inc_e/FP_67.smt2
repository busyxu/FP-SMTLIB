(declare-fun b_ack!1077 () (_ BitVec 64))
(declare-fun a_ack!1078 () (_ BitVec 64))
(assert (not (fp.lt ((_ to_fp 11 53) b_ack!1077) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!1077) ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.eq ((_ to_fp 11 53) a_ack!1078) ((_ to_fp 11 53) #x0000000000000000)))
(assert (not (fp.lt ((_ to_fp 11 53) b_ack!1077) ((_ to_fp 11 53) #xc085eeaad5511c64))))
(assert (fp.leq ((_ to_fp 11 53) b_ack!1077) ((_ to_fp 11 53) #xc024000000000000)))

(check-sat)
(exit)
