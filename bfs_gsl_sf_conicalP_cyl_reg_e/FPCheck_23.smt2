(declare-fun c_ack!89 () (_ BitVec 64))
(declare-fun a_ack!90 () (_ BitVec 32))
(declare-fun FPCHECK_FADD_UNDERFLOW ((_ BitVec 64) (_ BitVec 64)) Bool)
(assert (not (fp.leq ((_ to_fp 11 53) c_ack!89) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (bvslt a_ack!90 #xffffffff)))
(assert (not (= #xffffffff a_ack!90)))
(assert (not (= #x00000000 a_ack!90)))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!89) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!89) ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.lt ((_ to_fp 11 53) c_ack!89) ((_ to_fp 11 53) #x3ff0000000000000)))
(assert (FPCHECK_FADD_UNDERFLOW #x3ff0000000000000 c_ack!89))

(check-sat)
(exit)
