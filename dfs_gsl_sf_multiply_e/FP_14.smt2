(declare-fun a_ack!84 () (_ BitVec 64))
(declare-fun b_ack!83 () (_ BitVec 64))
(assert (not (fp.eq ((_ to_fp 11 53) a_ack!84) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) b_ack!83) ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.leq (fp.abs ((_ to_fp 11 53) a_ack!84))
        ((_ to_fp 11 53) #x3ff0000000000000)))
(assert (not (fp.geq (fp.abs ((_ to_fp 11 53) b_ack!83))
             ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (fp.leq (fp.abs ((_ to_fp 11 53) b_ack!83))
        ((_ to_fp 11 53) #x3ff0000000000000)))
(assert (not (fp.geq (fp.abs ((_ to_fp 11 53) a_ack!84))
             ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (fp.lt (fp.abs ((_ to_fp 11 53) a_ack!84))
            (fp.abs ((_ to_fp 11 53) b_ack!83)))))
(assert (fp.gt (fp.abs ((_ to_fp 11 53) a_ack!84)) (fp.abs ((_ to_fp 11 53) b_ack!83))))
(assert (fp.lt (fp.abs ((_ to_fp 11 53) a_ack!84)) ((_ to_fp 11 53) #x5feccccccccccccc)))
(assert (not (fp.eq (fp.abs ((_ to_fp 11 53) a_ack!84))
            ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.eq (fp.div roundNearestTiesToEven
               (fp.mul roundNearestTiesToEven
                       ((_ to_fp 11 53) a_ack!84)
                       ((_ to_fp 11 53) b_ack!83))
               ((_ to_fp 11 53) a_ack!84))
       ((_ to_fp 11 53) b_ack!83)))
(assert (not (fp.eq (fp.abs ((_ to_fp 11 53) b_ack!83))
            ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.eq (fp.div roundNearestTiesToEven
               (fp.mul roundNearestTiesToEven
                       ((_ to_fp 11 53) a_ack!84)
                       ((_ to_fp 11 53) b_ack!83))
               ((_ to_fp 11 53) b_ack!83))
       ((_ to_fp 11 53) a_ack!84)))
(assert (fp.lt (fp.abs (fp.mul roundNearestTiesToEven
                       ((_ to_fp 11 53) a_ack!84)
                       ((_ to_fp 11 53) b_ack!83)))
       ((_ to_fp 11 53) #x0010000000000000)))

(check-sat)
(exit)
