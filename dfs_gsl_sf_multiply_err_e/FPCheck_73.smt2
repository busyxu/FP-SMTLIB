(declare-fun a_ack!465 () (_ BitVec 64))
(declare-fun c_ack!464 () (_ BitVec 64))
(declare-fun FPCHECK_FMUL_OVERFLOW ((_ BitVec 64) (_ BitVec 64)) Bool)
(declare-fun b_ack!463 () (_ BitVec 64))
(assert (not (fp.eq ((_ to_fp 11 53) a_ack!465) ((_ to_fp 11 53) #x0000000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!464) ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.leq (fp.abs ((_ to_fp 11 53) a_ack!465))
        ((_ to_fp 11 53) #x3ff0000000000000)))
(assert (not (fp.geq (fp.abs ((_ to_fp 11 53) c_ack!464))
             ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (fp.leq (fp.abs ((_ to_fp 11 53) c_ack!464))
        ((_ to_fp 11 53) #x3ff0000000000000)))
(assert (not (fp.geq (fp.abs ((_ to_fp 11 53) a_ack!465))
             ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (fp.lt (fp.abs ((_ to_fp 11 53) a_ack!465))
       (fp.abs ((_ to_fp 11 53) c_ack!464))))
(assert (not (fp.gt (fp.abs ((_ to_fp 11 53) a_ack!465))
            (fp.abs ((_ to_fp 11 53) c_ack!464)))))
(assert (fp.lt (fp.abs ((_ to_fp 11 53) c_ack!464))
       ((_ to_fp 11 53) #x5feccccccccccccc)))
(assert (let ((a!1 (fp.lt (fp.abs (fp.mul roundNearestTiesToEven
                                  ((_ to_fp 11 53) a_ack!465)
                                  ((_ to_fp 11 53) c_ack!464)))
                  ((_ to_fp 11 53) #x0010000000000000))))
  (not a!1)))
(assert (FPCHECK_FMUL_OVERFLOW b_ack!463 c_ack!464))

(check-sat)
(exit)
