(declare-fun b_ack!1629 () (_ BitVec 32))
(declare-fun a_ack!1631 () (_ BitVec 32))
(declare-fun c_ack!1630 () (_ BitVec 64))
(declare-fun FPCHECK_FMUL_ACCURACY
             ((_ FloatingPoint 11 53) (_ FloatingPoint 11 53))
             Bool)
(assert (not (bvslt b_ack!1629 #x00000000)))
(assert (not (bvslt a_ack!1631 b_ack!1629)))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!1630) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!1630) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (= #x00000000 b_ack!1629))
(assert (not (bvslt a_ack!1631 #x00000000)))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!1630) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!1630) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (= #x00000000 a_ack!1631)))
(assert (not (= #x00000001 a_ack!1631)))
(assert (not (= #x00000002 a_ack!1631)))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!1630) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!1630) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (bvslt a_ack!1631 #x000186a0))
(assert (bvsle #x00000002 a_ack!1631))
(assert (FPCHECK_FMUL_ACCURACY
  (fp.mul roundNearestTiesToEven
          (fp.abs ((_ to_fp 11 53) c_ack!1630))
          ((_ to_fp 11 53) #x4008000000000000))
  (fp.mul roundNearestTiesToEven
          (fp.abs ((_ to_fp 11 53) c_ack!1630))
          ((_ to_fp 11 53) #x3cb0000000000000))))

(check-sat)
(exit)
