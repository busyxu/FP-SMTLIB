(declare-fun b_ack!1680 () (_ BitVec 32))
(declare-fun a_ack!1682 () (_ BitVec 32))
(declare-fun c_ack!1681 () (_ BitVec 64))
(declare-fun FPCHECK_FMUL_ACCURACY ((_ BitVec 64) (_ BitVec 64)) Bool)
(assert (not (bvslt b_ack!1680 #x00000000)))
(assert (not (bvslt a_ack!1682 b_ack!1680)))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!1681) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!1681) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (= #x00000000 b_ack!1680))
(assert (not (bvslt a_ack!1682 #x00000000)))
(assert (not (fp.lt ((_ to_fp 11 53) c_ack!1681) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (not (fp.gt ((_ to_fp 11 53) c_ack!1681) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (= #x00000000 a_ack!1682)))
(assert (not (= #x00000001 a_ack!1682)))
(assert (not (= #x00000002 a_ack!1682)))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!1681) ((_ to_fp 11 53) #x3ff0000000000000))))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!1681) ((_ to_fp 11 53) #xbff0000000000000))))
(assert (bvslt a_ack!1682 #x000186a0))
(assert (bvsle #x00000002 a_ack!1682))
(assert (bvsle #x00000003 a_ack!1682))
(assert (FPCHECK_FMUL_ACCURACY c_ack!1681 #x4014000000000000))

(check-sat)
(exit)
