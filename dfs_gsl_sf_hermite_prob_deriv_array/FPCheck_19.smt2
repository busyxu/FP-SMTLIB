(declare-fun b_ack!135 () (_ BitVec 32))
(declare-fun a_ack!137 () (_ BitVec 32))
(declare-fun c_ack!136 () (_ BitVec 64))
(declare-fun FPCHECK_FMUL_OVERFLOW ((_ FloatingPoint 11 53) (_ BitVec 64)) Bool)
(assert (not (bvslt b_ack!135 #x00000000)))
(assert (not (bvslt a_ack!137 #x00000000)))
(assert (not (= #x00000000 b_ack!135)))
(assert (not (= #x00000001 b_ack!135)))
(assert (= #x00000000 a_ack!137))
(assert (not (bvslt b_ack!135 #x00000000)))
(assert (not (= #x00000000 b_ack!135)))
(assert (not (= #x00000001 b_ack!135)))
(assert (not (fp.eq ((_ to_fp 11 53) c_ack!136) ((_ to_fp 11 53) #x0000000000000000))))
(assert (fp.gt (fp.abs ((_ to_fp 11 53) c_ack!136))
       ((_ to_fp 11 53) #x3ff0000000000000)))
(assert (bvslt #x00000001 b_ack!135))
(assert (fp.gt (fp.abs ((_ to_fp 11 53) c_ack!136))
       (fp.div roundNearestTiesToEven
               ((_ to_fp 11 53) #x7feccccccccccccc)
               (fp.abs ((_ to_fp 11 53) c_ack!136)))))
(assert (FPCHECK_FMUL_OVERFLOW (fp.abs ((_ to_fp 11 53) c_ack!136)) #x3cb0000000000000))

(check-sat)
(exit)
